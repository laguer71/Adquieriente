...
{
   public class VerificarTarjetahabienteTest
   {
      [Fact]
      public void VerificarTarjetahabiente()
      {
         Token token;
         token = Program.GetAuthToken();

         var endpoint = new Uri("https://api-dev.banorte.com/dev/ecommerce/v1/transacciones/tarjeta-habiente");
         var body = JsonConvert.SerializeObject(Tarjetahabiente.CrearBody());
